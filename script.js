...
const Inventory = {
    data: JSON.parse(localStorage.getItem('ONEN_INV_V74')) || [],
    save() {
        ...
        localStorage.setItem('ONEN_INV_V74', JSON.stringify(this.data));
        this.render();
    },
    exportCSV() {
    ...
    render() {
        ...
        list.innerHTML = this.data.map(p => `
            <div onclick="Core.prepItem('${p.sku}')" class="bg-white/5 p-4 rounded-2xl border border-white/5 cursor-pointer hover:border-amber-500 transition-all">
                <div class="text-[8px] text-amber-500 font-mono font-bold mb-1 opacity-40">[${p.sku}]</div>
...
const Core = {
    seq: 3400, activeIdx: 0, pending: null,
    ...
    async askGemini() {
        ...
        const promptText = `Extract from: "${rawText}". 
        Inv: ${JSON.stringify(Inventory.data)}. 
        Rules: 60 Dhaka, 70 Suburban, 120 Out. 
        ...
        } catch (e) { alert("AI parsing failed. Check input."); }
    ...
    injectRow(col, sz, qty) {
        ...
        const row = `
            <tr class="border-b border-slate-100">
                <td class="py-4 text-slate-300"></td>
    ...
    updateBatchList() {
        ...
        list.innerHTML = Array.from(document.querySelectorAll('.memo-card')).map((c, i) => `
            <div onclick="Core.selectMemo(document.querySelectorAll('.memo-card')[${i}])" class="p-4 bg-white/5 rounded-2xl text-[10px] cursor-pointer border ${i==this.activeIdx?'border-amber-500 bg-amber-500/10 font-bold':'border-transparent'}">
                ${c.querySelector('.m-id').innerText} — ${c.querySelector('.m-name').value || 'New Entry'}
            </div>`).join('');
    },
    exportPDF() {
